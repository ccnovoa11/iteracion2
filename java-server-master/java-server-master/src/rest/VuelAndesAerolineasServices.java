...
public class VuelAndesAerolineasServices {
	...
	@Path("/name/{name}")
	@Produces({ MediaType.APPLICATION_JSON })
	public Response getAerolineaName(@javax.ws.rs.PathParam("name") String name) {
		VuelAndesMaster tm = new VuelAndesMaster(getPath());
		ListaAerolineas aerolineas;
		try {
...
//	}
	...
	public Response updateAerolinea(Aerolinea aerolinea) {
		...
		return Response.status(200).entity(aerolinea).build();
	}
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.APPLICATION_JSON)
	public Response addAerolinea(ListaAerolineas aerolineas) {
		VuelAndesMaster tm = new VuelAndesMaster(getPath());
		try {
			tm.addAerolineas(aerolineas);;
		} catch (Exception e) {
			return Response.status(500).entity(doErrorMessage(e)).build();
		}
		return Response.status(200).entity(aerolineas).build();
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.APPLICATION_JSON)
	public Response deleteAerolinea(Aerolinea aerolinea) {
		VuelAndesMaster tm = new VuelAndesMaster(getPath());
		try {
			tm.deleteAerolinea(aerolinea);
		} catch (Exception e) {
			return Response.status(500).entity(doErrorMessage(e)).build();
		}
		return Response.status(200).entity(aerolinea).build();
